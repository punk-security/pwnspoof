...
for session_count in session_length_tests:
    for max_sessions_per_user in max_sessions_per_user_tests:
        sessions = list(generate_sessions(
        ...
        print(f"testing session counts - {len(sessions)} is greater than {lower_bound} and less than {upper_bound}")
...
for log_windows_days in log_window_days_tests:
    ...
    print(f"... earliest session is {sessions[0].start_datetime} and latest is {sessions[-1].start_datetime}")
    ...
    start_times = list((x.start_datetime.timestamp() - sdt.timestamp()) for x in sessions)
    target_deviation_s = ((log_windows_days) * 24 * 60 * 60) / 4 # 4 being halfway to halfway (which is mean)
    deviation_s = numpy.std( start_times )
    print(f"... session start time deviation is {deviation_s:.0f} and should be {target_deviation_s:.0f} which is factor difference of {deviation_s / target_deviation_s:.2f}")
